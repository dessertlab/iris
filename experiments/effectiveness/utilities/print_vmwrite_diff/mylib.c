...
void get_seeds(seeds_t *seeds_all, seeds_exit_reason_t *seeds_exit_reason)
{
    ...
    fclose(fp);
    if (raw_to_seeds(size_buffer, buffer, seeds_all, seeds_exit_reason, PRINT_MODE))
        printf("error:rawa_to_seeds\n");
    free(buffer);
}
...
int raw_to_seeds(int size_buffer, uint64_t *buffer, seeds_t *seeds_all, seeds_exit_reason_t *seeds_exit_reason, int mode)
{
    ...
    { // for each exit reason
        ...
        { // foreach read/write within an exit reason
            if (mode == INJECTION_MODE)
                type = buffer[indexes[i] + j + 5];
            ...
            if (type != 0)
            {
                ...
                if (data[count].field == 0x0004402)
                {
                    ...
                    if (seeds_items[i].exit_reason==28) c_r_access++;
                }
