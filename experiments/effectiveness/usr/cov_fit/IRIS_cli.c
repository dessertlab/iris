...
int main(int argc, char * argv[]) {
  ...
  case 'v': {
    unsigned int num_seeds = atoi(argv[2]); // num exits
    ...
    for (int i = 0; i < num_seeds; i++) {
      ...
      // Enables monitoring mode and allocate the output buffer
      ...
      uint64_t * buffer = malloc(sizeof(uint64_t) * BUFFER_DIM_FACTOR * 1);
      memset(buffer, 0, sizeof(uint64_t) * BUFFER_DIM_FACTOR * 1);
      res = xc_vmcs_fuzzing(pxch, dom_id, VMCS_BOOT_MONITORING_STOP, BUFFER_DIM_FACTOR * 1, buffer);
  ...
  case 'm': {
    ...
    for (int j = 0; j < seeds -> size; j++) {
      ...
      // Filter HLT
      if (exit_reason == 12 || exit_reason == 49) {
        printf("\n\n\nSEED injection: #%d, SEED DISCARDED, exit: %s\n\n\n", j, exit_reason_name[exit_reason]);
      } else {
        printf("SEED injection: #%d, SEED #%lu, exit: %ld\n", j, seeds -> seeds_items[j].id, exit_reason);
