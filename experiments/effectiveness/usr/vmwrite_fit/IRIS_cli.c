...
int main(int argc, char* argv[])
{
	...
	switch(*argv[1]){
	case 'v':{
		unsigned int num_exit = atoi(argv[2]);	 	// exit granularity 
		...
		xc_vmcs_fuzzing(pxch, 0, VMCS_BOOT_MONITORING_SET_NUM_SEC, 0, NULL);


		// Executes num_slot BOOT exits in monitoring mode saving it on file

		// Enables boot monitoring mode and allocate the output buffer
                xc_vmcs_fuzzing(pxch, 0, VMCS_BOOT_MONITORING_SET_EXIT_N, num_exit, NULL);
		xc_vmcs_fuzzing(pxch, 0, VMCS_BOOT_MONITORING_SETUP, BUFFER_DIM_FACTOR * num_exit, NULL);

		system("xl restore ./hvm_configuration.cfg ./guest_snapshot");
		...
		memset(buffer, 0, sizeof(uint64_t) * BUFFER_DIM_FACTOR * num_exit);
		res = xc_vmcs_fuzzing(pxch, dom_id, VMCS_BOOT_MONITORING_STOP, BUFFER_DIM_FACTOR * num_exit, buffer);

		// Writes seeds to file
		sprintf(file_name, "./seeds_record");
	...
	case 'm':{
		unsigned int exit_step = atoi(argv[2]); 		// num exit
		/*
		+ monitoring granularity, how much exits you want to record respect initial exits. 
		*/
		int gran_monitoring = atoi(argv[3]); 	
		...
		int size_monitoring = 3 * BUFFER_DIM_FACTOR * gran_monitoring;
		printf("size monitoring: %d\n", size_monitoring);


		// Start monitoring
		xc_vmcs_fuzzing(pxch, dom_id, VMCS_MONITORING_START, size_monitoring, NULL);
                nanosleep(&tim, &tim2);
		...
		for (int j=0; j<seeds->size; j++){
			...
			// Filter EPT MISCONFIG and VMCALL
			if (exit_reason != 49 && exit_reason != 18 && exit_reason != 12) {
				...
				printf("BOOT injection: #%d, SEED DISCARDED, exit: %s\n", j, exit_reason_name[exit_reason]);
			}
		...
		printf("\n\n\nSTOP MONITORING\n\n\n");
		nanosleep(&tim, &tim2);
		uint64_t *buffer_mon = malloc(sizeof(uint64_t) * size_monitoring);
		memset(buffer_mon, 0, sizeof(uint64_t) * size_monitoring);
		res = xc_vmcs_fuzzing(pxch, dom_id, VMCS_MONITORING_STOP, size_monitoring, buffer_mon);
		sprintf(file_name2, "./seeds_replay");
		if ((fp_mon = fopen(file_name2, "a")) == NULL) return -1;
		for (int k=0; k<res; k++) fprintf(fp_mon,"%"PRIx64"\n", buffer_mon[k]);
		fclose(fp_mon);
		free(buffer_mon);
	...
	case 'p':{
		...
		static uint64_t buffer[8 * BUFFER_DIM_FACTOR * 90000];
//		uint64_t *buffer = malloc(sizeof(uint64_t)*BUFFER_DIM_FACTOR * exit_step);
		printf("size: %ld\n", sizeof(uint64_t));

		uint64_t exit_reason = 0;
		static uint64_t size_buffer = 0;
		FILE* fp;
		int i, j;
