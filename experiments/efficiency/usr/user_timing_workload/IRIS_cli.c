...
*/
	...
	case 'v':{
		unsigned int exit_step = atoi(argv[2]);	 	// exit granularity		
		...
		xc_vmcs_fuzzing(pxch, 0, VMCS_DEBUG_MODE_DISABLE, 0, NULL);
		 ...
		 xc_vmcs_fuzzing(pxch, 0, VMCS_BOOT_MONITORING_SET_NUM_SEC, 10000000, NULL);
		...
		// Waits for the end of monitoring
		  ...
		  // Retrieve coverage after the injection of num_exit
                 sprintf(cmd, "xencov read > ./cov_mon/guest_workload_recording.dat");
                 system(cmd);
		...
		// Writes seeds to file
                sprintf(file_name, "./seeds_mon/guest_workload_recording_seeds");
		...
		uint64_t buffer_timestamps [exit_step];
                ...
                if((fp = fopen("./timestamps_recording.txt", "w"))==NULL) return -1;
...
//				printf("ret: %ld\n", res);
			...
			// Retrieve coverage after the injection of num_exit
			sprintf(cmd, "xencov read > ./cov_inj/guest_workload_replaying.dat");
			system(cmd);
		}
		 uint64_t buffer_timestamps [exit_step];
                int num_res=xc_vmcs_fuzzing(pxch,dom_id , VMCS_TIMING_MUTATION_STOP, exit_step, buffer_timestamps);
		if((fp = fopen("./timestamps_replaying.txt", "w"))==NULL) return -1;
                for (int i=0;i<exit_step;i++){
                        fprintf(fp,"%"PRIu64"\n", buffer_timestamps[i]-buffer_timestamps[0]);
                }
		fclose(fp);
